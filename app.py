...
SPREADSHEET_ID = "1yPTtr0dK6ncqPcR0k2M_eO-3YBzU68wVA5ISM5YDSFc"
...
def save_member_to_sheet(member_data):
    ...
    try:
        ...
        try:
            ...
            ws = sh.add_worksheet(title="회원목록", rows=1000, cols=30)
            ws.append_row(["이름","나이","성별","키","몸무게","BMI","직업","대사질환",
                           "운동목적","검사부위","수술이력","수술경과주","저장일시"])
        # 기존 회원 있으면 업데이트, 없으면 추가
        records = ws.get_all_records()
        name = member_data.get("name","")
        for i, row in enumerate(records):
            if row.get("이름") == name:
                row_num = i + 2  # 헤더 포함
                ws.update(f"A{row_num}:M{row_num}", [[
                    name,
                    member_data.get("age",""),
                    member_data.get("sex",""),
                    ...
                    ", ".join(member_data.get("metabolic",[])),
                    ", ".join(member_data.get("goal_types",[])),
                    ", ".join(member_data.get("body_parts",[])),
                    member_data.get("surgery_name",""),
                    member_data.get("surgery_weeks_ago",""),
        ...
        for row in records:
            ...
            if name:
                members[name] = {
                    ...
                    "age": row.get("나이", 30),
                    "sex": row.get("성별","남"),
                    "height": row.get("키", 170),
                    "weight": row.get("몸무게", 70),
                    "bmi": row.get("BMI", 0),
                    "occupation": row.get("직업",""),
                    "metabolic": [x.strip() for x in str(row.get("대사질환","")).split(",") if x.strip()],
                    "goal_types": [x.strip() for x in str(row.get("운동목적","")).split(",") if x.strip()],
                    "body_parts": [x.strip() for x in str(row.get("검사부위","")).split(",") if x.strip()],
                    "surgery_name": row.get("수술이력",""),
                    "surgery_weeks_ago": row.get("수술경과주", 0),
...
- 우수: 70 이하 / 양호: 71~90 / 보통: 91~110 / 미흡: 111 이상""","video":"https://www.youtube.com/results?search_query=3+minute+step+test+cardiorespiratory"},
        {"id":"D2","name":"Push-Up Test","goal":"상체 근지구력 (연령·성별 등급 평가)","method":"""
**자세 기준:**
1. 손은 어깨너비, 손가락은 앞을 향함
2. 몸은 머리부터 발뒤꿈치까지 일직선 유지
3. 가슴이 바닥에서 5cm 이내로 내려오기
4. 1분간 최대 횟수 측정 (자세 무너지면 중단)

**여성 변형:** 무릎을 바닥에 대고 시행 가능

**양성 판정:** 아래 기준표에서 D등급 이하

**등급 기준표 (ACSM 기준, 출처: ACSM's Guidelines for Exercise Testing and Prescription, 2021):**

| 등급 | 20대남 | 30대남 | 40대남 | 50대남 | 20대여 | 30대여 | 40대여 | 50대여 |
|------|--------|--------|--------|--------|--------|--------|--------|--------|
| A (우수) | 47+ | 39+ | 30+ | 25+ | 36+ | 30+ | 23+ | 18+ |
| B (양호) | 37-46 | 30-38 | 22-29 | 17-24 | 27-35 | 21-29 | 15-22 | 11-17 |
| C (보통) | 29-36 | 22-29 | 17-21 | 13-16 | 21-26 | 15-20 | 13-14 | 7-10 |
| D (미흡) | 22-28 | 17-21 | 13-16 | 10-12 | 15-20 | 11-14 | 9-12 | 5-6 |
| F (불량) | 21이하 | 16이하 | 12이하 | 9이하 | 14이하 | 10이하 | 8이하 | 4이하 |""","video":"https://www.youtube.com/results?search_query=push+up+fitness+test"},
        {"id":"D3","name":"허리둘레 측정","goal":"복부비만 평가 (한국인 기준)","method":"""
**측정 방법:**
...
**한국인 허리둘레 기준 (출처: 대한비만학회 비만진료지침 2022):**
...
**추가 참고 (WHO 아시아태평양 기준):**
- 남성 90cm / 여성 80cm 이상: 대사증후군 위험 증가
- 허리-엉덩이 비율(WHR): 남성 0.90 초과, 여성 0.85 초과 시 위험

**양성 판정:** 남성 90cm 이상 / 여성 85cm 이상""","video":"https://www.youtube.com/results?search_query=waist+circumference+measurement+technique"},
    ],
    "체형교정": [
...
def generate_csv_full(patient_info, test_results, week_records, custom_exercises):
    ...
    # ── 헤더 ──────────────────────────────────────────────────────────────────
    w.writerow(["AI 맞춤형 운동재활 시스템 - 회원 기록지"])
    w.writerow([f"출력일: {datetime.now().strftime('%Y년 %m월 %d일 %H:%M')}"])
    w.writerow([])
    ...
    w.writerow(["=" * 40])
    field_labels = {
        "환자명":"이름", "나이":"나이", "키":"키", "몸무게":"몸무게",
        "BMI":"BMI", "직업":"직업", "대사질환":"대사질환",
        "부위":"검사 부위", "VAS":"통증 강도(VAS)", "강도":"운동 강도",
        "수술이력":"수술 이력", "기록일":"기록일"
    }
    ...
    for tid, res in test_results.items():
        w.writerow([tid, res])
        if "양성" in res: pos_count += 1
    w.writerow(["양성 합계", f"{pos_count}개 / {len(test_results)}개"])
    w.writerow([])

    # ── 운동 기록 ─────────────────────────────────────────────────────────────
    if week_records:
        w.writerow(["=" * 40])
        w.writerow(["▶ 주차별 운동 기록"])
        w.writerow(["=" * 40])
        for week, exercises in sorted(week_records.items()):
            w.writerow([f"── {week}주차 ──"])
            w.writerow(["운동명", "수행여부", "세트/횟수", "특이사항"])
            for ex, data in exercises.items():
                w.writerow([ex, data.get("done",""), data.get("sets",""), data.get("notes","")])
    ...
    w.writerow(["─" * 40])
    w.writerow(["본 기록지는 AI 운동재활 시스템에서 자동 생성되었습니다."])
    w.writerow(["전문 의료인의 지도 하에 활용하시기 바랍니다."])
...
with st.sidebar:
    ...
    # 페이지 로드할 때마다 구글 시트에서 자동 불러오기
    if "sheet_loaded" not in st.session_state:
        with st.spinner("회원 목록 불러오는 중..."):
            sheet_members = load_members_from_sheet()
            if sheet_members:
                st.session_state.members.update(sheet_members)
                ...
                try:
                    sh = get_gsheet()
                    if sh is None:
                        st.warning("⚠️ 구글 시트 연결 실패 — Secrets 설정 확인 필요")
                except Exception as e:
                    st.error(f"연결 오류: {e}")
        st.session_state.sheet_loaded = True
    ...
    with col_r1:
        if st.button("새로고침", use_container_width=True):
            with st.spinner("불러오는 중..."):
                ...
                if sheet_members:
                    st.session_state.members.update(sheet_members)
                    st.success(f"✅ {len(sheet_members)}명!")
                else:
                    st.info("저장된 회원 없음")

    if st.session_state.members:
        # 검색 기능
        search_query = st.text_input("회원 검색", placeholder="이름 검색...", label_visibility="collapsed")
        st.caption("이름으로 검색")

        all_names = list(st.session_state.members.keys())
        if search_query:
            filtered = [n for n in all_names if search_query.lower() in n.lower()]
        else:
            filtered = all_names

        if filtered:
            member_names = ["-- 선택 --"] + filtered
            sel_mem = st.selectbox("회원 선택", member_names, label_visibility="collapsed")
            if sel_mem != "-- 선택 --":
                m = st.session_state.members[sel_mem]
                st.caption(f"저장일: {m.get('saved_at','')}")
                if st.button("이 회원 불러오기", use_container_width=True):
            ...
            st.caption(f"'{search_query}' 검색 결과 없음")
    st.markdown("---")
    ...
    # 체형교정 선택 시 안내
    if "체형교정" in goal_types:
        st.caption("📐 체형교정 검사(자세평가·측만증 등)가 자동 포함돼요")
    ...
    st.markdown("---")
    if st.button("💾 회원 저장 (구글 시트)", use_container_width=True):
        if patient_name:
            member_data = {
                ...
                "surgery_name":st.session_state.get("surgery_name",""),
            ...
            # 세션에 저장
            st.session_state.members[patient_name] = member_data
            # 구글 시트에 저장
            with st.spinner("구글 시트에 저장 중..."):
                ok = save_member_to_sheet(member_data)
            if ok:
                st.success(f"✅ {patient_name} 구글 시트에 저장 완료!")
            else:
                st.warning(f"✅ {patient_name} 세션에 저장됨 (시트 연결 확인 필요)")
        else:
...
if "다이어트" in goal_types:
    ...
    diff = round(weight - high_w, 1)
    st.markdown(f"""
<div class="card card-blue" style="margin-top:10px;margin-bottom:0px">
<b>📊 BMI 분석</b>
...
with tab1:
    ...
    with st.expander("수술 이력 입력 (선택사항 — 수술 재활인 경우)", expanded=False):
        ...
        if surgery_name == "기타 (직접 입력)":
            st.markdown("**수술명 직접 입력**")
            surgery_custom = st.text_input("수술명 직접 입력",
                                           value=st.session_state.get("surgery_custom",""),
        ...
        if surgery_name != "없음/해당없음":
            ...
            with col_s1:
                ...
                surgery_date = st.date_input("수술 날짜",
                                             value=st.session_state.get("surgery_date", datetime.now().date()),
                                             max_value=datetime.now().date(),
                                             label_visibility="collapsed")
                st.session_state.surgery_date = surgery_date
                # 수술 후 경과 주수 자동 계산
                surgery_weeks = max(0, (datetime.now().date() - surgery_date).days // 7)
                st.session_state.surgery_weeks_ago = surgery_weeks
                st.caption(f"수술 후 약 **{surgery_weeks}주** 경과")
            with col_s2:
                st.markdown("**수술 측**")
            ...
            if protocol_text:
                st.markdown(f'<div style="background:#fef9c3;border-left:4px solid #f59e0b;border-radius:8px;padding:10px 14px;font-size:0.82rem;margin-top:8px"><b>현재 적용 프로토콜 ({surgery_weeks}주차):</b><br>{protocol_text.replace(chr(10),"<br>")}</div>', unsafe_allow_html=True)
        ...
        for test in rec_tests:
            with st.expander(f"🔬 **[{test['id']}] {test['name']}**", expanded=True):
                ...
                with col_r:
                    ...
                    no_bilateral_prefixes = ("D","P")
                    is_bilateral = (
                        any(tid.startswith(p) for p in bilateral_prefixes) and
                        not any(tid.startswith(p) for p in no_bilateral_prefixes)
                    )

                    # 특수 등급 검사: 심폐지구력 (3분 스텝 - 회복 심박수 기준)
                    if tid == "D1":
                        age_g = patient_age
                        sex_g = patient_sex
                        st.caption("회복 심박수(1분 후) 기준")
                        if sex_g == "남":
                            if age_g < 30:   opts = ["A 우수 (79↓)","B 양호 (80-84)","C 보통 (85-93)","D 미흡 (94-100)","F 불량 (101↑)"]
                            elif age_g < 40: opts = ["A 우수 (83↓)","B 양호 (84-90)","C 보통 (91-97)","D 미흡 (98-103)","F 불량 (104↑)"]
                            elif age_g < 50: opts = ["A 우수 (87↓)","B 양호 (88-93)","C 보통 (94-101)","D 미흡 (102-107)","F 불량 (108↑)"]
                            else:            opts = ["A 우수 (90↓)","B 양호 (91-97)","C 보통 (98-104)","D 미흡 (105-112)","F 불량 (113↑)"]
                        else:
                            if age_g < 30:   opts = ["A 우수 (85↓)","B 양호 (86-92)","C 보통 (93-100)","D 미흡 (101-107)","F 불량 (108↑)"]
                            elif age_g < 40: opts = ["A 우수 (88↓)","B 양호 (89-96)","C 보통 (97-103)","D 미흡 (104-110)","F 불량 (111↑)"]
                            elif age_g < 50: opts = ["A 우수 (93↓)","B 양호 (94-99)","C 보통 (100-107)","D 미흡 (108-115)","F 불량 (116↑)"]
                            else:            opts = ["A 우수 (95↓)","B 양호 (96-102)","C 보통 (103-110)","D 미흡 (111-118)","F 불량 (119↑)"]
                        prev_grade = st.session_state.test_results.get(tid, opts[2])
                        idx = opts.index(prev_grade) if prev_grade in opts else 2
                        grade = st.radio("등급 선택", opts, index=idx, key=f"grade_{tid}", label_visibility="collapsed")
                        results[tid] = grade
                        color = "#16a34a" if grade.startswith("A") or grade.startswith("B") else \
                                "#f59e0b" if grade.startswith("C") else "#dc2626"
                        st.markdown(f'<span style="background:{color};color:white;border-radius:6px;padding:3px 10px;font-weight:700">{grade[:1]}등급</span>', unsafe_allow_html=True)
                    ...
                    # 특수 등급 검사: Push-Up
                        ...
                        sex_g = patient_sex
                        if sex_g == "남":
                            if age_g < 30:   opts = ["A (47회+)","B (37-46회)","C (29-36회)","D (22-28회)","F (21회-)"]
                            elif age_g < 40: opts = ["A (39회+)","B (30-38회)","C (22-29회)","D (17-21회)","F (16회-)"]
                            elif age_g < 50: opts = ["A (30회+)","B (22-29회)","C (17-21회)","D (13-16회)","F (12회-)"]
                            else:            opts = ["A (25회+)","B (17-24회)","C (13-16회)","D (10-12회)","F (9회-)"]
                        else:
                            if age_g < 30:   opts = ["A (36회+)","B (27-35회)","C (21-26회)","D (15-20회)","F (14회-)"]
                            elif age_g < 40: opts = ["A (30회+)","B (21-29회)","C (15-20회)","D (11-14회)","F (10회-)"]
                            elif age_g < 50: opts = ["A (23회+)","B (15-22회)","C (13-14회)","D (9-12회)","F (8회-)"]
                            else:            opts = ["A (18회+)","B (11-17회)","C (7-10회)","D (5-6회)","F (4회-)"]
                        prev_grade = st.session_state.test_results.get(tid, opts[2])
                        idx = opts.index(prev_grade) if prev_grade in opts else 2
                        grade = st.radio("등급 선택", opts, index=idx, key=f"grade_{tid}", label_visibility="collapsed")
                        results[tid] = grade
                        color = "#16a34a" if grade.startswith("A") or grade.startswith("B") else \
                                "#f59e0b" if grade.startswith("C") else "#dc2626"
                        st.markdown(f'<span style="background:{color};color:white;border-radius:6px;padding:3px 10px;font-weight:700">{grade}</span>', unsafe_allow_html=True)

                    # 특수 검사: 허리둘레
                    elif tid == "D3":
                        st.caption("기준: 남 90cm↑ / 여 85cm↑ 복부비만")
                        prev = st.session_state.test_results.get(tid, "정상 (음성)")
                        result = st.radio("결과", ["복부비만 (양성)", "정상 (음성)"],
                                          index=0 if prev=="복부비만 (양성)" else 1,
                                          key=f"grade_{tid}", label_visibility="collapsed")
                        results[tid] = result
                        badge = '<span class="pos">복부비만</span>' if "양성" in result else '<span class="neg">정상</span>'
...
<div class="ex-card">
  ...
  <div style="font-size:0.78rem;color:#7c3aed;margin-top:6px">💡 {ex.get('tip','')}</div>
        ...
        # 전체 기록 저장
        st.markdown("---")
        st.markdown("#### 전체 기록 다운로드")
        csv_data = generate_csv_full(
            {"환자명":patient_name,"나이":f"{patient_age}세","키":f"{height}cm",
             "몸무게":f"{weight}kg","BMI":bmi_val,"직업":occ_detail or occ,
             "대사질환":", ".join(metabolic) or "없음",
             "부위":", ".join(body_parts),"VAS":f"{st.session_state.vas}/10",
             "강도":intensity_label,
             "수술이력":st.session_state.get("surgery_name","없음"),
        ...
        st.download_button("📥 전체 기록 다운로드", data=csv_data, file_name=fname, mime="text/csv", use_container_width=True)
        generated_weeks = sorted(st.session_state.prescription_weeks.keys())
        if generated_weeks:
            st.caption(f"생성된 운동 프로그램: {', '.join(str(w)+'주차' for w in generated_weeks)}")
